...
public class NewsController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (newsModel.getType().equalsIgnoreCase(Constant.LIST)) {
			...
			if(newsModel.getAlert() != null && newsModel.getMessage() != null) {
				req.setAttribute(Constant.ALERT, bundle.getString(newsModel.getAlert()));
				req.setAttribute(Constant.MESSAGE, bundle.getString(newsModel.getMessage()));
			}
			...
			if (newsModel.getId() != null) {
				...
				req.setAttribute(Constant.TITLE, "chỉnh sửa bài viết");
			} else {
				req.setAttribute(Constant.TITLE, "thêm bài viết");
			}
